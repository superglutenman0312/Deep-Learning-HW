...
TARGET_UPDATE = 25              #每隔幾回合去更新目標網路的權重
TOTAL_TIMESTEPS = 1000           #總訓練的回合數 (原本1000)
VISUALIZE = True                #是否在訓練過程中渲染遊戲畫面 顯示遊戲畫面
MAX_STAGNATION_STEPS = 400       # Max steps without x_pos change 500
...
MODEL_PATH = os.path.join(".","step_181_X_max_2020.pth")        # 模型權重檔案的存放路徑
# ========== 載入模型權重 =========== 
...
for timestep in tqdm(range(1, TOTAL_TIMESTEPS + 1), desc="Training Progress"):  #主訓練迴圈，進行TOTAL_TIMESTEPS次迭代
    ...
    while not done:
        ...
        # custom_reward = no_move_punish(info, custom_reward, prev_info)
    ...
    print(f"total_coin_reward:{total_coin_reward}, total_x_offset_reward:{total_x_offset_reward}, total_y_offset_reward:{total_y_offset_reward}")
    print(f"total_monster_reward:{total_monster_reward}, total_man_reward:{total_man_reward}, total_flag_reward:{total_flag_reward}, total_pipe_reward:{total_pipe_reward}, total_pipe_reward2:{total_pipe_reward2}")
    print("-----------------------------------------end--------------------------------------------------------------------------------------------------------------------------------------------")
    ...
    if cumulative_custom_reward > best_reward:
        ...
        model_path = os.path.join("ckpt_test",f"step_{timestep}_reward_{int(cumulative_reward)}_custom_{int(cumulative_custom_reward)}.pth")
