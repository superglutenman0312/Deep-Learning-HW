...
def distance_y_offset_reward(info, reward, prev_info):
    total_reward = reward
    y_offset_reward = 0
    y_change = info['y_pos'] - prev_info['y_pos']
    if y_change >= 0:
        y_offset_reward = y_change * 2
    else:
        y_offset_reward = y_change * -1
...
def distance_x_offset_reward(info, reward, prev_info):
    total_reward = reward
    x_offset_reward = 0
    x_change = info['x_pos'] - prev_info['x_pos']
    if x_change > 0: #前進
        x_offset_reward = x_change * 1
    elif x_change < 0: #後退
        x_offset_reward = x_change * -2
    else:
        x_offset_reward = -2
...
def monster_score_reward(info, reward, prev_info):
    total_reward = reward
    score_reward = 0
    if info['score'] - prev_info['score'] == 100 and info['coins'] == prev_info['coins']: #代表踩死一隻怪
        score_reward = 1000
...
def pipe_jump_reward(info, reward, prev_info):
    ...
    # 假設水管的位置範圍
    pipe_x_start = 722 #898
    pipe_y_top = 145  # 水管的高度（假設）
...
def no_move_punish(info, reward, prev_info):
    total_reward = reward
    if info['x_pos'] - prev_info['x_pos'] == 0 and info['y_pos'] - prev_info['y_pos'] == 0:
        total_reward -= 1
    return total_reward
